...
def monthly_rest_sample_hashes(archives: tuple[RawArchive, ...]) -> frozenset[str]:
    """Select the first archive present in each UTC calendar month deterministically."""
    if not archives:
        return frozenset()
...
def build_provider_verification_evidence(
    ...
    *,
    ...
    monthly_rest_hashes = (
        monthly_rest_sample_hashes(archives)
        if policy is VerificationPolicy.CHECKSUM_ALL_REST_MONTHLY
        else frozenset(archive.sha256 for archive in archives)
    )

    evidence: list[ProviderVerificationEvidence] = []
    for archive in archives:
        if archive.sha256 in monthly_rest_hashes:
