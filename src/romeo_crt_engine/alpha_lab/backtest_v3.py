...
from romeo_crt_engine.alpha_lab.models_v3 import V3BacktestResult, V3Config, V3Signal
...
def _trade_from_signal(
    ...
    *,
    ...
    risk_distance = signal.atr * config.stop_atr
    if risk_distance <= 0 or risk_distance >= entry_reference:
        return None
    stop = entry_reference - risk_distance
    target = entry_reference + (risk_distance * config.target_r)
...
def run_v3_backtest(
    ...
    *,
    ...
    if costs is None:
        costs = CostModel()
    signals = generate_v3_signals(bars, config)
    trades: list[Trade] = []
    equity = initial_equity
    next_signal_index = 0
