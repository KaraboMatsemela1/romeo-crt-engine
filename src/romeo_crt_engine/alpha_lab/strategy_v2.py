...
from romeo_crt_engine.alpha_lab.models_v2 import V2Config, V2Signal
...
def generate_v2_signals(bars: Sequence[Bar], config: V2Config) -> tuple[V2Signal, ...]:
    warmup = max(config.lookback_hours, config.atr_period + 1, config.ema_slow)
    ...
    for index in range(len(bars) - 1):
        ...
        long_signal = long_allowed and close > reference_high * (1.0 + buffer_rate)
        short_signal = short_allowed and close < reference_low * (1.0 - buffer_rate)
        ...
        signals.append(
            V2Signal(
                ...
                atr=atr_value,
                reference_level=reference_high if long_signal else reference_low,
            )
        )
